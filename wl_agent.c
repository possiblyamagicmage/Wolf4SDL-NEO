...
void ControlMovement (objtype *ob)
{
    ...
    if(buttonstate[bt_strafeleft] && !doscontrols)
    {
    ...
    if(buttonstate[bt_straferight] && !doscontrols)
    {
        angle = ob->angle - ANGLES/4;
        if(angle < 0)
            angle += ANGLES;
    ...
    // side to side move
    //
    if ((buttonstate[bt_strafeleft] || buttonstate[bt_straferight]) && doscontrols)
    {
        //
        // strafing
        //
...
{
    // don't make game depend on sound playback
    if(facetimes > 0)
    {
        facetimes--;
        return;
